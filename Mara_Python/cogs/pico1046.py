...
class Ilyal(Cog, name='Ilyal', description="Those commands that are only meant to be used with love purposes and between loved ones."):
    ...
    async def pico1046(self, context: Context) -> None:
    ...
    async def ily(self, context: Context) -> None:
    ...
    async def surprise(self, context: Context) -> None:
    ...
    async def myname(self, context: Context, name: str=None) -> None:
        """Gives the baby bot a new name or asks for their names."""
        names: ObservedDict = db['name']
        if name is None:
            name: list = list(names)[0]
            if context.author.id in _family_ids:
    ...
    async def pronoun(self, context: Context) -> None:
    ...
    async def show(self, context:Context, key: str=None) -> None:
        """Prepares a bot for a surprise gift."""
        if key is None:
            message: str = "Here are all the possible arguments!"
